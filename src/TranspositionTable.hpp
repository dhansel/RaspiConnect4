...
class TableGetter {
 private:
  virtual void* getKeys() { return NULL; }
  virtual void* getValues() { return NULL; }
  virtual size_t getSize() { return 0; }
  virtual int getKeySize() const { return 0; }
  virtual int getValueSize() const { return 0; }

 public:
  virtual value_t get(key_t key) const { return (value_t) 0; }
  virtual void put(key_t key, value_t value) {}
  virtual bool isCollision(key_t key) const { return false; }
  virtual ~TableGetter() {};
...
class TranspositionTable : public TableGetter<key_t, value_t> {
 private:
  ...
  int getKeySize()   const override {return sizeof(partial_key_t);}
  int getValueSize() const override {return sizeof(value_t);}
 ...
 public:
  ...
  bool isCollision(key_t key) const override {
    size_t pos = index(key);
    return (K[pos] !=0 && K[pos] != (partial_key_t)key); 
  }
